...
CompositeMap::CompositeMap(const Map &map1_, const Map &map2_)
	: Map(map1_.domain(), map2_.codomain()),
	  map1(map1_),
	  map2(map2_),
	  middle(new float [map1_.codomain().dim()])
{
}
...
void
CompositeMap::apply(const float in[], float out[]) const
{
...
void
CompositeMap::apply_inverse(const float in[], float out[]) const
{
...
void
CompositeMap::apply_derivative(const Point &origin,
