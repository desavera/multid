...
public:
	...
	class PreimageNotFound : public std::exception { };
...
inline
Pairing::Pairing(const GeomObj & preimage, const GeomObj & image)
	: head(new PairingNode(preimage, image, 0, 0)),
	  tail(head)
{
}
...
inline void
Pairing::append(const GeomObj & preimage, const GeomObj & image)
{
	PairingNode *node = tail;
	tail = new PairingNode(preimage, image, 0, node);
	if (node == 0)
		head = tail;
	else
		node->next = tail;
...
inline void
Pairing::prepend(const GeomObj & preimage, const GeomObj & image)
{
	PairingNode *node = head;
	head = new PairingNode(preimage, image, node, 0);
	if (node == 0)
		tail = head;
	else
		node->prev = head;
...
{
	if (p != 0)
		p = Pix(((PairingNode *) p)->prev);
}

inline void
Pairing::add(const GeomObj & preimage, const GeomObj & image)
{
	append(preimage, image);
}

inline GeomObj
Pairing::image(const GeomObj & preimage)
{
...
inline GeomObj
Pairing::preimage(const GeomObj & image)
{
	for (PairingNode *p = tail; p != 0; p = p->prev)
		if (p->image == image)
			return p->preimage;
