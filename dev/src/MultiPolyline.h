	  ...
	  coords_array(0)
{
	...
	for (Pix p = actual.list.first(); p != 0; actual.list.next(p)) {
		Polyline pline(map, (Polyline &) actual.list(p));
		list.append(pline);
	}
}
...
public:
	...
	class IncompatibleSpaces : public std::exception { };

protected:
	ActualMultiPolyline * const actual_multi_polyline(void) const;
	ActualMultiPolyline * actual_multi_polyline(void);
...
inline
MultiPolyline::MultiPolyline(const Map &map, const MultiPolyline &multi_polyline)
...
inline ActualMultiPolyline * const
MultiPolyline::actual_multi_polyline(void) const
{
	return (ActualMultiPolyline *) actual;
}

inline ActualMultiPolyline *
MultiPolyline::actual_multi_polyline(void)
{
	return (ActualMultiPolyline *) actual;
}
