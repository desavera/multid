	  ...
	  color_array(0)
{
	...
	for (Pix p = actual.list.first(); p != 0; actual.list.next(p)) {
		Polygon pgon(map, (Polygon &) actual.list(p));
		list.append(pgon);
	}
}
...
protected:
	ActualMultiPolygon * const actual_multi_polygon(void) const;
	ActualMultiPolygon * actual_multi_polygon(void);
...
inline ActualMultiPolygon * const
MultiPolygon::actual_multi_polygon(void) const
{
	return (ActualMultiPolygon *) actual;
}

inline ActualMultiPolygon *
MultiPolygon::actual_multi_polygon(void)
{
	return (ActualMultiPolygon *) actual;
}
...
#endif
	make_actual();
	actual_multi_polygon()->list
		.append(multi_polygon.actual_multi_polygon()->list);
