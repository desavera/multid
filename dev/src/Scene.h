...
#endif
	for (Pix p = actual_scene.list.first();
			p != 0; actual_scene.list.next(p)) {
...
protected:
	ActualScene * const actual_scene(void) const;
...
inline ActualScene * const
Scene::actual_scene(void) const
{
	return (ActualScene *) actual;
}
