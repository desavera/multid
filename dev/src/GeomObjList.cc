...
GeomObjList::operator[](int i) const
{
	...
	if (i >= current_index) {
		while (current_index < i) {
			((GeomObjList * const) this)->current = current->next;
			((GeomObjList * const) this)->current_index++;
		}
		return current->obj;
	}
	else {
		while (current_index > i) {
			((GeomObjList * const) this)->current = current->prev;
			((GeomObjList * const) this)->current_index--;
		}
		return current->obj;
	}
