...
void
Plot2dScalingMap::apply(const float in[], float out[]) const
{
...
void
Plot2dScalingMap::apply_inverse(const float in[], float out[]) const
{
...
void
Plot2dScalingMap::get_range(float &x_min_, float &x_max_,
...
void
Plot2dScalingMap::set_range(float x_min_, float x_max_,
		float y_min_, float y_max_)
{
	...
	x_offset = x_min;
	x_scale = (square_window ? 1. : GOLDEN_RATIO)/(x_max - x_min);
	y_offset = y_min;
...
Plot2dSettings::reset(void)
{
	...
	x_range.value(oss_x.str().c_str());

	std::ostringstream oss_y;
	oss_y << y_min_ << " " << y_max_ << std::ends;
	y_range.value(oss_y.str().c_str());
}

Plot2d::Plot2d(const Space &space,
		const char * const x_name,
		const char * const y_name,
		const Frame &parent,
		int x, int y, int width, bool square_window)
	: View2d(plot_2d_space, parent, Panel::HEIGHT_ONE_ROW, x, y,
		(int) (width/(square_window ? 1. : GOLDEN_RATIO)), width),
	  scaling_map_(space, square_window),
	  background_scene_(space),
	  ...
	  reset_button(panel(), plot_2d_settings)
{
	Bounds2d bounds2d;
	bounds2d.xmin = 0.;
	bounds2d.xmax = (square_window ? 1. : GOLDEN_RATIO);
	bounds2d.ymin = 0.;
	bounds2d.ymax = 1.;
	bounds(bounds2d);
}
...
void
Plot2d::get_range(float &x_min_, float &x_max_,
		float &y_min_, float &y_max_) const
{
...
void
Plot2d::set_range(float x_min_, float x_max_, float y_min_, float y_max_)
{
