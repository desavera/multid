...
public:
	...
	virtual void apply_inverse(const float in[], float out[]) const;
	virtual void apply_derivative(const Point &origin,
		const float in[], float out[]) const;
	virtual const Attributes *
		image_attributes(const GeomObj &geom_obj) const;

	class NotInvertible : public std::exception { };
