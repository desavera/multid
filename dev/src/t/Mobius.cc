...
Mobius::Mobius(const Frame &parent)
	: View2d(mobius_space, parent, Panel::HEIGHT_ONE_ROW,
		GUI::default_x, GUI::default_y,
		GUI::default_height,
		(int) (GUI::default_height/M_PI_2) + 1),
	  euclid2(0),
	  euclid3(0),
...
void
Mobius::button_proc(Button button, float /* x */, float y)
{
...
void
Mobius::drag_proc(Button button, float /* x */, float y)
{
	if ((button & BUTTON_SELECT) && (button & BUTTON_NO_MODIFIER)) {
		Polyline polyline(space(), 2);
		polyline[0][0] = -1.;
...
void
Mobius::leftkey_proc(int key, float /* x */, float /* y */)
{
	if (key == 4 /* UNDO */) {
		empty();
		if (euclid2 != 0)
